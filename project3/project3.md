...
Adaptive Sampling is an algorithm that avoids excessive amount of sampling per pixel, which is pretty common and necessary for Monte Carlo method to avoid noise. Adaptive sampling determines when to stop sampling with statistical approach. Assuming that n samples have been traced so far, in order to figure out if we should stop sampling, we can calculate the mean $$\mu$$ and the standard deviation $$\sigma$$. And we define a variable $$I=1.96 \times \frac{\sigma}{\sqrt{n}}$$. The sampling should be stopped if $$I \leq maxTolerence \cdot \mu$$, where $$maxTolerence$$ is typically 0.05.

This basically says that the 1-side length of the 95% confidence level should be less than $$0.05 \cdot \mu$$. The shorter the confidence interval, the more confident we are that the true value would be close to our estimation. We are not sure why we multiply 0.05 with $$\mu$$, our assumption is if the pixel is already very bright, then it is more likely that the pixel has already converged. However, if the pixel is dark (i.e., a small $$\mu$$), then we need to make sure that the place is actually a black spot and not because of noise (note: noise is usually presented as a black spot).

Our implementation of the Adaptive Sampling algorithm is the following:

We keep `raytrace_pixel` almost the same, except now we add logic to break early due to Adaptive Sampling

1. First initializes variables `s1` and `s2` to keep track of the sum of the sample light and the sum of the square of the sample light, respectively. The variable `num_sample` will keep track of the number of samples taken for the current pixel.
3. The adaptive sampling algorithm checks if the current sample is a multiple of `samplesPerBatch`. If so, it computes the mean and variance of the sample light using `s1` and `s2`. It then computes the confidence interval of the mean using the formula $$I=1.96 \times \frac{\sigma}{\sqrt{n}}$$. If the confidence interval is less than or equal to the maximum tolerance `maxTolerance` times the mean, the loop breaks and the average light of the pixel is returned.
4. Otherwise, ray trace and compute the light at the pixel exactly as before, and update `s1` and `s2`

The rest is the same as before.

Here are some example noise-free images we made using adaptive sampling. The maximum sample per pixel is set at 2024, 1 ray per light, maximum bounce of 5 and sample per batch is set at 64 with max tolerance set at 0.05 (i.e., `-s 2048 -l 1 -m 5 -a 64 0.05`)

| Scene | Result | Rate |
|-------|--------|------|
| `sky/CBbunny.dae`              | <img src="./images/p5_CBbunny.png" style="width:100%"/>              | <img src="./images/p5_CBbunny_rate.png" style="width:100%"/> |
| `sky/CBspheres_lambertian.dae` | <img src="./images/p5_CBsphere.png" style="width:100%"/> | <img src="./images/p5_CBsphere_rate.png" style="width:100%"/> |
